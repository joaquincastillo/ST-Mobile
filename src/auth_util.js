...
export const getHeaders = () =>
  Promise.all([
    ...
    SecureStore.getItemAsync("actualPropertyId")
  ]).then(values => ({
    token: values[0],
...
export const setOtherProperty = ({ propertyId, communityId }) =>
  Promise.all([
    SecureStore.setItemAsync("actualCommunityId", communityId),
    SecureStore.setItemAsync("actualPropertyId", propertyId)
  ]);

export const signIn = newToken =>
  Promise.all([
    SecureStore.setItemAsync(AUTH_TOKEN, newToken.token),
